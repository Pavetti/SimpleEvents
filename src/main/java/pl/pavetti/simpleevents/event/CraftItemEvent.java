        ...
        if(running){
            ...
            if(clickType.isShiftClick()) {
                ...
                for(ItemStack actualItem : Inventory.getContents()){ //For each item in crafting inventory{
                    if(actualItem.getType() != Material.AIR && actualItem.getType() != Material.CAVE_AIR && actualItem.getType() != Material.VOID_AIR
                            && lowerAmount > actualItem.getAmount() && !actualItem.getType().equals(craftedItem.getType())) //if slot is not air && lowerAmount is highter than this slot amount && it's not the recipe amount
