    ...
    public void load() {
        ...
        if (configuration == null) {
            throw new NoCorrectEventDataException(
                "Can not load event data cause eventData.yml is empty. Try to rebuild this file."
            );
        }

        for (String section : configuration.getKeys(false)) {
            //creating list of prize items form file
            List<Map<String, Object>> itemsMapList = (List<
                    Map<String, Object>
                >) configuration.getList(section + ".prizeItems");
